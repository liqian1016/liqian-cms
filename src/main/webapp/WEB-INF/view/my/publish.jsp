	...
	<script>
		...
		$(function(){
			...
			$.post("/article/selectsChannel",{},function(json){
				for(var i in json){
					$("[name='channelId']").append("<option value='"+json[i].id+"'>"+json[i].name+"</option>");
				}
			},"json");
			...
			$("[name='channelId']").change(function(){
				$.post("/article/selectsCategory",{id:$(this).val()},function(json){
					...
					for(var i in json){
						$("[name='categoryId']").append("<option value='"+json[i].id+"'>"+json[i].name+"</option>");
					}
				},"json");
			})
...
<body>
	...
	<form name="example" id="f1">
		...
		分类:<select name="categoryId" class="form-control">
			<option>---请选择分类---</option>
		</select>
		</div>
		<br>
